...
using namespace std;
using namespace SCFT::VirtualDisplay;
using namespace Microsoft::WRL;

#pragma region SampleMonitors

static constexpr DWORD SCFT_VIRTUAL_DISPLAY_MONITOR_COUNT = 1; // If monitor count > ARRAYSIZE(s_SampleMonitors), we create edid-less monitors

// Default modes reported for edid-less monitors. The first mode is set as preferred
static const struct ScftVirtualDisplayMonitor::SampleMonitorMode s_SampleDefaultModes[] = 
{
    { 2560, 1440, 60 },
    { 1920, 1080, 60 },
    { 1280,  720, 60 },
...
static const struct ScftVirtualDisplayMonitor s_SampleMonitors[] =
{
...
EVT_WDF_DRIVER_DEVICE_ADD SCFTVirtualDisplayDeviceAdd;
EVT_WDF_DEVICE_D0_ENTRY SCFTVirtualDisplayDeviceD0Entry;

EVT_IDD_CX_ADAPTER_INIT_FINISHED SCFTVirtualDisplayAdapterInitFinished;
EVT_IDD_CX_ADAPTER_COMMIT_MODES SCFTVirtualDisplayAdapterCommitModes;

EVT_IDD_CX_PARSE_MONITOR_DESCRIPTION SCFTVirtualDisplayParseMonitorDescription;
EVT_IDD_CX_MONITOR_GET_DEFAULT_DESCRIPTION_MODES SCFTVirtualDisplayMonitorGetDefaultModes;
EVT_IDD_CX_MONITOR_QUERY_TARGET_MODES SCFTVirtualDisplayMonitorQueryModes;

EVT_IDD_CX_MONITOR_ASSIGN_SWAPCHAIN SCFTVirtualDisplayMonitorAssignSwapChain;
EVT_IDD_CX_MONITOR_UNASSIGN_SWAPCHAIN SCFTVirtualDisplayMonitorUnassignSwapChain;
...
_Use_decl_annotations_
NTSTATUS SCFTVirtualDisplayDeviceAdd(WDFDRIVER Driver, PWDFDEVICE_INIT pDeviceInit)
{
    ...
    PnpPowerCallbacks.EvtDeviceD0Entry = SCFTVirtualDisplayDeviceD0Entry;
    ...
    // IddConfig.EvtIddCxDeviceIoControl = SCFTVirtualDisplayIoDeviceControl;

    IddConfig.EvtIddCxAdapterInitFinished = SCFTVirtualDisplayAdapterInitFinished;

    IddConfig.EvtIddCxParseMonitorDescription = SCFTVirtualDisplayParseMonitorDescription;
    IddConfig.EvtIddCxMonitorGetDefaultDescriptionModes = SCFTVirtualDisplayMonitorGetDefaultModes;
    IddConfig.EvtIddCxMonitorQueryTargetModes = SCFTVirtualDisplayMonitorQueryModes;
    IddConfig.EvtIddCxAdapterCommitModes = SCFTVirtualDisplayAdapterCommitModes;
    IddConfig.EvtIddCxMonitorAssignSwapChain = SCFTVirtualDisplayMonitorAssignSwapChain;
    IddConfig.EvtIddCxMonitorUnassignSwapChain = SCFTVirtualDisplayMonitorUnassignSwapChain;
...
_Use_decl_annotations_
NTSTATUS SCFTVirtualDisplayDeviceD0Entry(WDFDEVICE Device, WDF_POWER_DEVICE_STATE PreviousState)
{
...
void SwapChainProcessor::WriteFrame(IDXGIResource* resource)
{
    ULONGLONG now = GetTickCount64();
    if (now - m_lastFrameWritten < 33)
    {
        return;
    }
    ...
    bool recreateStaging = !m_stagingTexture
    ...
    if (recreateStaging)
    {
        if (FAILED(m_Device->Device->CreateTexture2D(&stagingDescription, nullptr, &m_stagingTexture)))
        {
    ...
    m_Device->DeviceContext->CopyResource(m_stagingTexture.Get(), source.Get());

    D3D11_MAPPED_SUBRESOURCE mapped = {};
    if (FAILED(m_Device->DeviceContext->Map(m_stagingTexture.Get(), 0, D3D11_MAP_READ, 0, &mapped)))
    {
        return;
    }

    const DWORD rowBytes = description.Width * 4;
    const DWORD imageBytes = rowBytes * description.Height;
    ...
    imageHeader.biHeight = -static_cast<LONG>(description.Height);
    imageHeader.biPlanes = 1;
    imageHeader.biBitCount = 32;
    imageHeader.biCompression = BI_RGB;
    imageHeader.biSizeImage = imageBytes;

    CreateDirectoryW(L"C:\\ProgramData\\SCFT", nullptr);
    HANDLE file = CreateFileW(L"C:\\ProgramData\\SCFT\\virtual-display-frame.tmp", GENERIC_WRITE, 0, nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr);
    ...
    m_frameBuffer.resize(imageBytes);
    for (DWORD row = 0; row < description.Height; row++)
    {
        const BYTE* sourceRow = static_cast<const BYTE*>(mapped.pData) + static_cast<size_t>(row) * mapped.RowPitch;
        CopyMemory(m_frameBuffer.data() + static_cast<size_t>(row) * rowBytes, sourceRow, rowBytes);
    }

    bool written = write(&fileHeader, sizeof(fileHeader))
        && write(&imageHeader, sizeof(imageHeader))
        && write(m_frameBuffer.data(), imageBytes);

    m_Device->DeviceContext->Unmap(m_stagingTexture.Get(), 0);
    CloseHandle(file);

    if (written && MoveFileExW(L"C:\\ProgramData\\SCFT\\virtual-display-frame.tmp", L"C:\\ProgramData\\SCFT\\virtual-display-frame.bmp", MOVEFILE_REPLACE_EXISTING))
    {
        m_lastFrameWritten = now;
    }
}
...
void IndirectDeviceContext::InitAdapter()
{
    ...
    AdapterCaps.MaxMonitorsSupported = SCFT_VIRTUAL_DISPLAY_MONITOR_COUNT;
    AdapterCaps.EndPointDiagnostics.Size = sizeof(AdapterCaps.EndPointDiagnostics);
    AdapterCaps.EndPointDiagnostics.GammaSupport = IDDCX_FEATURE_IMPLEMENTATION_NONE;
    AdapterCaps.EndPointDiagnostics.TransmissionType = IDDCX_TRANSMISSION_TYPE_WIRED_OTHER;

    // Declare your device strings for telemetry (required)
    AdapterCaps.EndPointDiagnostics.pEndPointFriendlyName = L"SCFTVirtualDisplay Device";
    AdapterCaps.EndPointDiagnostics.pEndPointManufacturerName = L"Microsoft";
    AdapterCaps.EndPointDiagnostics.pEndPointModelName = L"SCFTVirtualDisplay Model";
...
void IndirectDeviceContext::FinishInit(UINT ConnectorIndex)
{
    ...
    {
        MonitorInfo.MonitorDescription.DataSize = ScftVirtualDisplayMonitor::szEdidBlock;
...
_Use_decl_annotations_
NTSTATUS SCFTVirtualDisplayAdapterInitFinished(IDDCX_ADAPTER AdapterObject, const IDARG_IN_ADAPTER_INIT_FINISHED* pInArgs)
{
    ...
    {
        for (DWORD i = 0; i < SCFT_VIRTUAL_DISPLAY_MONITOR_COUNT; i++)
        {
...
_Use_decl_annotations_
NTSTATUS SCFTVirtualDisplayAdapterCommitModes(IDDCX_ADAPTER AdapterObject, const IDARG_IN_COMMITMODES* pInArgs)
{
...
_Use_decl_annotations_
NTSTATUS SCFTVirtualDisplayParseMonitorDescription(const IDARG_IN_PARSEMONITORDESCRIPTION* pInArgs, IDARG_OUT_PARSEMONITORDESCRIPTION* pOutArgs)
{
    // ==============================
    ...
    pOutArgs->MonitorModeBufferOutputCount = ScftVirtualDisplayMonitor::szModeList;

    if (pInArgs->MonitorModeBufferInputCount < ScftVirtualDisplayMonitor::szModeList)
    {
    ...
    else
    {
        ...
        if (pInArgs->MonitorDescription.DataSize != ScftVirtualDisplayMonitor::szEdidBlock)
            return STATUS_INVALID_PARAMETER;

        DWORD SampleMonitorIdx = 0;
        for(; SampleMonitorIdx < ARRAYSIZE(s_SampleMonitors); SampleMonitorIdx++)
        {
            if (memcmp(pInArgs->MonitorDescription.pData, s_SampleMonitors[SampleMonitorIdx].pEdidBlock, ScftVirtualDisplayMonitor::szEdidBlock) == 0)
            {
                // Copy the known modes to the output buffer
                for (DWORD ModeIndex = 0; ModeIndex < ScftVirtualDisplayMonitor::szModeList; ModeIndex++)
                {
...
_Use_decl_annotations_
NTSTATUS SCFTVirtualDisplayMonitorGetDefaultModes(IDDCX_MONITOR MonitorObject, const IDARG_IN_GETDEFAULTDESCRIPTIONMODES* pInArgs, IDARG_OUT_GETDEFAULTDESCRIPTIONMODES* pOutArgs)
{
...
_Use_decl_annotations_
NTSTATUS SCFTVirtualDisplayMonitorQueryModes(IDDCX_MONITOR MonitorObject, const IDARG_IN_QUERYTARGETMODES* pInArgs, IDARG_OUT_QUERYTARGETMODES* pOutArgs)
{
    UNREFERENCED_PARAMETER(MonitorObject);
...
_Use_decl_annotations_
NTSTATUS SCFTVirtualDisplayMonitorAssignSwapChain(IDDCX_MONITOR MonitorObject, const IDARG_IN_SETSWAPCHAIN* pInArgs)
{
...
_Use_decl_annotations_
NTSTATUS SCFTVirtualDisplayMonitorUnassignSwapChain(IDDCX_MONITOR MonitorObject)
{
    auto* pMonitorContextWrapper = WdfObjectGet_IndirectMonitorContextWrapper(MonitorObject);
