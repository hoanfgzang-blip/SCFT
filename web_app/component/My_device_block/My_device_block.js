...
function renderConnectingDevice(devices) {
...
async function initDevices() {
    try {
        ...
        const savedDeviceId = localLoader('SCFT_ActiveDevice');

        if (savedDeviceId != null) {
            currentDevices.forEach(d => {
                if (savedDeviceId !== "NONE" && d.id === savedDeviceId && d.status === "online") {
                    d.using = true;
                } else {
                    d.using = false;
                }
